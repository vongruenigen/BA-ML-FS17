...
\paragraph{Surprising Results} The results on the test set are quite the opposite of the results from the training process (see Figures~\ref{result:test_performance:opensubtitles} and~\ref{result:test_performance:reddit}), because both of the models are getting worse over time. The results of the OpenSubtitles model (see Table~\ref{results:test_metrics:opensubtitles}) seem to vary across the different checkpoints, with the best result having a perplexity of $71.07$ and a loss of $6.15$ and coming from the evaluation with the first checkpoint. The best result of the Reddit model (see Table~\ref{results:test_metrics:reddit}) is also achieved on the first checkpoint with a perplexity of $169.14$, with all other checkpoints having a worse perplexity.
\\
\begin{table}[H]
	\centering
	\begin{adjustbox}{max width=\textwidth}
		...
		\end{tabular}
	\end{adjustbox}
	\caption{Loss and perplexity values for each snapshot of the OpenSubtitles model when evaluating them against the test dataset.}
...
\begin{table}[H]
	...
	\begin{adjustbox}{max width=\textwidth}
		...
		\end{tabular}
	\end{adjustbox}
	\caption{Loss and perplexity values for each snapshot of the Reddit model when evaluating them against the test dataset.}
