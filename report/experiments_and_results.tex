...
The behaviour is the same for the uni-gram distributions, which is the reason we put this visualizations in the Appendix~\ref{APPENDIX UNIGRAM OPUS}.
...
As said in the last Chapter, we are now going to analyze the language diversity of the outputs the two models produce over the different snapshots. For this purpose, we are analyzing the how much the top 10 uni-, bi-gram and sentences
...
\paragraph{Fazit}
Im Vergleich zu Cleverbot schneidet unser Modell gleichgut oder besser ab (subjektiv), Test war schwer für Cleverbot weil die Aussagen anhand der responses unseres Modells ausgewählt haben. 
Beim Vergleich mit den verschiednen NCM Utterance Aussagentypen scheint es so, dass unsere Modelle für einfachere Fragen etwa gleichgut funbktioniert und für komplexere Fragen aber die Sprachvielfalt vermutlich noch fehlt. (und gröösse Modell)
