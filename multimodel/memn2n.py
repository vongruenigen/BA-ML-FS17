...
#              The implementation itself is a copy of the implementation
#              by domluna found at https://github.com/domluna/memn2n.
#
...
class MemN2N(model.Base):
    ...
    def __init__(self, cfg, session):
        '''intializes the model with given config and session.'''
        self.cfg = self.DEFAULT_PARAMS.copy()
        self.__deep_merge(self.cfg, cfg)
        self.mcfg = self.cfg.get('model_config')

        self._batch_size = self.mcfg.get('batch_size')
        self._sentence_size = sentence_size
        self._memory_size = self.mcfg.get('memory_size')
        self._embedding_size = self.mcfg('embeddings').shape[0]
        self._hops = self.cf.get('hops')
        self._max_grad_norm = self.mcfg.get('gradient_clipping_norm')
        self._nonlin = self.mcfg.get('non_linearity')
        self._init = tf.random_normal_initializer(stddev=0.1)
        self._name = 'memn2n'
        self._encoding = self.__position_encoding
    ...
    def __position_encoding(sentence_size, embedding_size):
    ...
    def __zero_nil_slot(t, name=None):
        """
    ...
    def __add_gradient_noise(t, stddev=1e-3, name=None):
        """
