...
def truncate(text, width=30, token='...'):
    '''Truncates the given text to the given width and ends it
       with the set end_token.'''
    return textwrap.shorten(text, width=width, placeholder=token)
...
def random_sequences(length_from, length_to, vocab_lower, vocab_upper, batch_size, as_string=True):
