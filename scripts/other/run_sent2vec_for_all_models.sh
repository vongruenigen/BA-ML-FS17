...
do
  ...
  do
    ...
    S2V_RESULTS_COSINE="$MODEL/test_s2v_cosine_similarity.json"
    S2V_RESULTS_EUCLIDEAN="$MODEL/test_s2v_euclidean_similarity.json"

    if [ -f "$S2V_EXPECTED" ]; then
      echo "Skipping $MODEL as the s2v file already exists!"
      continue
    fi

    echo "Starting to generate s2v embeddings for the opensubitles model $MODEL..."
    python scripts/generate_s2v_sequence_embeddings.py misc/fasttext misc/$S2V_MODEL_NAME 700 2 $PREDS_FILE $S2V_GENERATED $S2V_EXPECTED
  done
  ...
  do
    ...
    if [ -f "$S2V_GENERATED" ]; then
      echo "Skipping $MODEL as the s2v file already exists!"
      continue
    fi

    echo "Starting to generate s2v embeddings for the reddit model $MODEL..."
    python scripts/generate_s2v_sequence_embeddings.py misc/fasttext misc/$S2V_MODEL_NAME 700 2 $PREDS_FILE $S2V_GENERATED $S2V_EXPECTED
  done
done
