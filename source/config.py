...
class Config(object):
    ...
    DEFAULT_PARAMS = {
        ...
        # Legacy option for old configs, use "devices" instead.
        'device': None,

        # Default device(s) to use. Multiple devices may be used by the
        # model if supported, all other computations then run on the
        # first in the list, which is considered the "default" device.
        'devices': ['gpu:0'],

        # An internal setting used to keep track of the current device
        # when iterating over the list of available devices.
        'device_idx': None,
    ...
    def __init__(self, cfg_obj={}):
        ...
        if self.cfg_obj['device'] is not None:
            self.cfg_obj['devices'][0] = self.cfg_obj['device']
