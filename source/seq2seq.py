...
def _extract_argmax_and_embed(embedding,
  ...
  def loop_function(prev, _):
    if output_projection is not None:
      ...
      if len(output_projection) == 3 and output_projection[2] is not None:
...
def attention_decoder(decoder_inputs,
  ...
  Args:
    ...
    hidden_state_projection: If set, this variable will be used to reduce the
      size of the final hidden state which is then passed to the classifcation
      layer.
  ...
  if hidden_state_projection is not None:
      outputs = [math_ops.matmul(outputs[i], hidden_state_projection) for i in range(len(outputs))]
...
def beam_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "attention_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if i == 0:
          states = [state for _ in range(beam_size)]
          state = tf.reshape(tf.concat(states, 0), [-1, state_size])
      ...
      if len(output_projection) == 3 and output_projection[2] is not None:
        output = math_ops.matmul(output, output_projection[2])
...
def embedding_attention_decoder(decoder_inputs,
  ...
  with variable_scope.variable_scope(
    ...
    if beam_search:
        loop_function = _extract_beam_search(embedding,
                                             beam_size,num_symbols,
                                             embedding_size,
                                             output_projection,
    ...
    if beam_search:
        return beam_attention_decoder(emb_inp,
                                      initial_state,
                                      attention_states,
                                      cell,
        ...
        if output_projection is not None and len(output_projection) == 3:
          hidden_state_projection = output_projection[2]
...
def decode_model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  The seq2seq argument is a function that defines a sequence-to-sequence model,
  e.g., seq2seq = lambda x, y: basic_rnn_seq2seq(x, y, core_rnn_cell.GRUCell(24))
  ...
  with ops.name_scope(name, "model_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
        ...
        bucket_outputs, _, beam_path, beam_symbol, log_beam_probs = \
            seq2seq(encoder_inputs[:bucket[0]], decoder_inputs[:bucket[1]])

        outputs.append(bucket_outputs)
        beam_paths.append(beam_path)
        beam_symbols.append(beam_symbol)
        log_beam_probs.append(log_beam_probs)

  return outputs, beam_paths, beam_symbols, log_beam_probs
